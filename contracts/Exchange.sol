...
contract Exchange is ERC20 {
  ...
  */
        if (cryptoDevTokenReserve == 0) {
            ...
            //is adding 'Eth' to the contract, so whatever 'Eth' contract has is equal to the one supplied by the user in the current 'addLiquidity' call
    ...
    function removeLiquidity(uint _amount) public returns (uint, uint) {
        ...
        // Transfer `ethAmount` of Eth from the contract to the users wallet
        payable(msg.sender).transfer(ethAmount);
        // Transfer `cryptoDevTokenAmount` of crypto dev tokens from the contract to the users wallet
